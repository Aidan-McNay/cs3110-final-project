...
(* let create_move_record piece start finish check captured_a_piece castle
   promoted checkmate en_passant = let piece_type = fst piece in let color = snd
   piece in let start = Utils.Location.init_loc (fst start) (snd start) in let
   finish = Utils.Location.init_loc (fst finish) (snd finish) in
   ...
   2,0-0-0,0-0\n\ 3,cxb1R+,axb1B\n\ 4,g8N+,f8Q\n\ 5,hxg3+,Qg3+\n\ 6,Rxa7,Na3\n\
   7,Qc3,Bg5\n\ 8,e3,bxa3 e.p.\n\ 9,dxe6# e.p.," Board.Alg_notation.(
   move_history_to_algformat (List.rev (List.map (move_record_to_alg_notation
   []) move_record_test_list)) 1) ); ( "empty list to string test" >:: fun _ ->
   assert_equal "" (Board.Alg_notation.move_history_to_algformat [] 1) ); ( "one
   move record to string test" >:: fun _ -> assert_equal "1,dxe6# e.p.,"
